...
export default function Page() {
  ...
  const [modelResetId, setModelResetId] = useState(0)
  ...
  const handleAvatarStateChange = useCallback((state: 'loading' | 'ready' | 'signing') => {
    setAvatarState(state)
    if (state !== 'loading') setIsResettingModel(false)
  }, [])

  const resetActiveModel = () => {
    setIsResettingModel(true)
    setModelResetId((current) => current + 1)
  }
          ...
          <div className="terminal-window" aria-label="Yap and Render live translation workspace">
            ...
            <div className="terminal-body avatar-terminal-body">
              <div className="avatar-canvas-shell" aria-label="Interactive 3D avatar canvas ready for Indian Sign Language playback">
                <AvatarPlayer phrase={avatarPhrase} requestId={requestId} model={avatarModel} appendToQueue={liveMode} stopId={stopId} resetId={modelResetId} speed={modelSpeed} onStateChange={handleAvatarStateChange} />
                <span className="canvas-badge"><span className="live-dot" /> ISL AVATAR · {avatarState}</span>
                <button type="button" className={isResettingModel ? 'model-reset is-resetting' : 'model-reset'} onClick={resetActiveModel} aria-label="Reset model" title="Reset Model" disabled={isResettingModel}><RefreshCw size={14} /><span>{isResettingModel ? 'Initializing…' : 'Reset'}</span></button>
